...
def game(player):
    ...
    while not finished:
        ...
        screen.blit(f1.render(str(sc2) + '/10', 1, BLACK), (550, 640))
...
def start_menu():
    ...
    На экране 3 кнопки:
        Play - запуск новой игры
        Best scores - высвечивается таблица с 10 лучшими результатами (по количеству монет)
        Exit - выход из игры
...
def die_menu(s, s1, s2):
    """
    Создает окно с результатом игры после встречи с крысой
    На экране 1 кнопка:
        OK - выход в главное меню

    Args:
        s - количество монет на момент завершения игры
        s1 - количество заданий по матану на момент завершения игры
        s2 - количество учебников Г. Е. Иванова на момент завершения игры
    ...
    while not back:
        ...
        screen.blit(f1.render(str(s2) + '/10', 1, BLACK), (100, 250))
...
def win_menu(s, s1, s2):
    """
    Создает окно с результатом игры после сдачи всех 3 заданий преподавателю
    На экране 1 кнопка:
        OK - выход в главное меню
...
def name_menu():
    """
    Создает окно для ввода имени перед началом игры
    На экране 1 кнопка:
        OK - начало игры
...
def best_scores():
    """
    Создает окно с 10 лучшими результатами (по количеству монет)
    На экране 1 кнопка:
        Back to menu - возвращение в главное меню
